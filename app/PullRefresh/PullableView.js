...
const defaultOnShouldRefresh = (resolve) => {
    //do something
    setTimeout(() => {
        resolve();
    }, 1000);
...
export default class extends PureComponent {
    ...
    componentDidMount() {
        ...
        this.state.scrollY.addListener((info) => {
            ...
            let direction = (info.value > 0 && info.value >= this.lastYDist)
                ? 'down'
                : 'up';
            if (this.state.loadMoreState.type != 'loading' && direction == 'down') {
                if (info.value + this.scrollViewHeight > this.scrollViewContentSize - this.props.loadMoreSwitchBottomMargin) {
                    this.setState({loadMoreState: {type: 'loading'}});
                    this.props.onShouldLoadMore && this.props.onShouldLoadMore(this.resolveLoadMoreHandler);
                }
            }
            this.lastYDist = info.value;
        });
    ...
    doRefresh = () => {
        ...
        setTimeout(() => {
            ...
            this.props.onShouldRefresh ? this.props.onShouldRefresh(this.resolvePullHandler) : defaultOnShouldRefresh(this.resolvePullHandler);
        }, this.props.duration);
    };
    ...
    onScrollEnd = () => {
        if (this.state.loadMoreState.type != 'loading') {
            if (this.lastYDist + this.scrollViewHeight >= this.scrollViewContentSize - this.props.loadMoreSwitchBottomMargin) {
                this.setState({loadMoreState: {type: 'loading'}});
                this.props.onShouldLoadMore && this.props.onShouldLoadMore(this.resolveLoadMoreHandler);
            }
        }
            ...
            <View style={{flex: 1}}>
                ...
                >
                    ...
                    <Animated.ScrollView
                        ...
                        onContentSizeChange={(w, h) => this.onScrollViewContentSizeChange(w, h)}
                        onScrollEndDrag={(e) => {
                            this.onScrollEnd()
                        }}
                    >
